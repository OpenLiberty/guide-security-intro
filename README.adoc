...
It is not recommended to store password in plain text. As you can see in the `identityStore.xml`, all user passwords are encrypted by the XOR encoding. For example, to encrypt 'bobpwd', navigate to the `target/liberty/wlp/bin` directory and run the Liberty
