...
The `user` page and `admin` page simply display user information. The `user` and `admin` pages show the name and role for the currently authenticated user.

Administrators have access to both pages, the `user` page and the `admin` page. Whereas the user will only have access to the `user` page.
...
that only administrator account such as bob can access to the admin. However, a normal user like alice can only see an `Error 403: AuthorizationFailed`
error message.
