...
Navigate your browser to this URL to access the application: http://localhost:9080[window="_blank"]
...
Point your browser to http://localhost:9080[window="_blank"]. As you can see, the browser gets redirected from an HTTP connection to an HTTPS connection automatically, which is due to the transport guarantee defined in the `HomeServlet` class.
...
[source, java, linenums, role="code_column"]
----
include::finish/src/test/java/it/io/openliberty/guides/security/SecurityTest.java[tags=**]
----
