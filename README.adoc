...
== What you'll learn
You'll learn how to secure a web application by performing authentication and authorization using Java EE Security. Authentication confirms the identity of the user by verifying a user name and password. While authorization determines whether a user has access to restricted resources.

Java EE Security provides capability to configure the basic, form or custom form authentication mechanism by using annotations in the servlets.
It also provides a SecurityContext API for programmatic security checks in the application. 

You’ll learn how to implement form authentication. Form authentication allows you to specify the web application login and error pages, assign the security constraint of a servlet and use the SecurityContext API to determine the role of a logged in user.
...
Upon hitting the initial link `http://localhost:9080`, the application automatically switches from `HTTP` to `HTTPS` connection, and forwards to a login page. If the browser gives you a certificate warning, it's because the Liberty server created an SSL certificate by default. You may follow your browser's provided instructions to accept the certificate and continue.

The identity store provided is defined with the following user information credentials:
...
Now you can sign in to the application with one of the credentials from the identity store.
...
When you sign in as Dave, you'll be blocked and a message will display `Error 403: AuthorizationFailed` because Dave doesn't have a role.

Once you're done checking out the application, logout and stop the Open Liberty server:
```
...
Typically, applications would use an external and configurable registry, like a database or Lightweight Directory Access Protocol (LDAP). However, for this guide, a basic registry provided by Liberty is used as a simple example of an identity store.

Create the `src/main/liberty/config/identityStore.xml` file:

[source, xml, indent=0]
----
include::finish/src/main/liberty/config/identityStore.xml[tags=**]
----
...
Within the `<application-bnd>` elements, two different authorization roles are assigned to these groups: `admin` and `user`.
...
It is not recommended to store password in plain text. For simplicity reasons, the application in this guide uses XOR encoding to encrypt the passwords, as you can see in the `identityStore.xml`. For example, to encrypt 'bobpwd', navigate to the `target/liberty/wlp/bin` directory and run the Liberty
...
The `transportGuarantee` attribute defines the data protection requirements that must be satisfied by the transport. In this case, it's set to the `CONFIDENTIAL` configuration, which specifies that all user data must be encrypted during transport. This attribute ensures that the browser gets redirected from `http` to an `https` link automatically.

The Java EE Security specification also defines a common SecurityContext API that can be used to perform some common programmatic security-related operations. You can get access to the information of the authenticated account by calling the appropriate methods from the SecurityContext API.
...
Use one of the credentials from the `Identity Store` table to log in.
...
while the `testAuthorizationFail()` method tests a user that exists in the identity store but isn't authorized to access the application.
