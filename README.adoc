...
You’ll learn how to implement form authentication. Form authentication allows you to specify the web application login and error pages. You'll also assign the security constraint of a servlet and use the SecurityContext API to determine the role of a logged in user.
...
The following table shows the user identities which are stored in the Liberty user registry:
...
Now you can sign in to the application with one of the credentials from the user registry.
...
Use one of the credentials from the `User Registry` table to log in.
...
Now you can sign in to the `HomeServlet` with one of the credentials from the user registry.
...
while the `testAuthorizationFail()` method tests a user that exists in the user registry but isn't authorized to access the application.
