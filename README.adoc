...
which is why an HTTP connection from a browser will switch to HTTPS.
...
access information in the registry for authentication and authorization using APIs like the Jakarta EE Security API.
...
[hotspot=user-carl file=0]`carl` and [hotspot=user-dave file=0]`dave`, and four groups, 
...
Map the groups in the [hotspot file=0]`userRegistry.xml` to the appropriate user roles supported by the application for proper user authorization,
using the [hotspot=Security file=1]`<security-role/>` and 
[hotspot=Group hotspot=group-name-Employee file=1]`<group/>` elements. 
