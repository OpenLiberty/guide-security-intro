...
Upon hitting the initial link `http://localhost:9080`, the application automatically switches from `HTTP` to `HTTPS` connection, and forwards to a login page. 
...
When you sign in as Alice, you can only view Alice's name. Furthermore, there is a person named Dave that belongs to a group called `PartTime` but has no role. 
When you sign in as Dave, you will be blocked and a message will display `Error 403: AuthorizationFailed`.
...
Typically, applications would use an external and configurable registry, like a database or Lightweight Directory Access Protocol (LDAP). However, we will be using the basic registry provided by Liberty, as a simple example of an identity store for the purposes of this guide.
...
Use the Liberty `securityUtility` command to encrypt the passwords so that they will not appear as plain text in the `server.xml` file. Navigate to `target/liberty/wlp/bin` directory, and run the following:
```
./securityUtility encode --encoding=xor alicepwd

```

Here, `alice`, `carl`, and `dave` use the XOR encoding for their passwords. While `bob` uses Advanced Encryption Standard (AES) encryption, an encryption key is required to encrypt and decrypt the password.

```
./securityUtility encode --encoding=aes --key=secret bobpwd
```

The encryption key must be provided to the server by defining the variable `wlp.password.encryption.key`. 
This property is set in a configuration file which is saved outside of the configuration directory for the server. This ensures that the file containing the encryption key will not be included when running the server dump or package command. In this application, the configuration file is saved at `src/main/liberty/config/pwdEncryptionKey.xml`.
...
In order to enable form authentication to the `HomeServlet`, define `@FormAuthenticationMechanismDefinition` annotation and set its loginToContinue attribute to accept a configured `@LoginToContinue` annotation which makes `welcome.html` as the login page and `error.html` as the error page.
...
The `HomeServer.doGet()` method uses the `securityContext.isCallerInRole()` API to determine the current user's role  and forward the repsonse to a proper page.
