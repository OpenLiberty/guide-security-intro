...
It also provides a SecurityContext API for programmatic security checks in Java EE containers. You’ll learn how to implement form authentication. Form authentication allows you to specify the web application login and error pages, assign the security constraint of a servlet and use the SecurityContext API to determine the role of a logged in user.
...
As you can see in the `identityStore.xml`, all user passwords are encrypted by the XOR encoding. For example, to encrypt 'bobpwd', navigate to the `target/liberty/wlp/bin` directory and run the Liberty
...
To restrict the access of different JSF resources, the `web.xml` defines the admin and user roles in the `<security-role>` tags and defines the web resource and authorization constraint in the `<security-constraint>` tags. The `user.jsf` and `admin.jsf` pages can only be accessed by whoever has user or admin roles respectively.
