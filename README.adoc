...
The `transportGuarantee` attribute defines the data protection requirements that must be satisfied by the transport. In this case, it's set to the `CONFIDENTIAL` configuration, which specifies that all user data must be encrypted during transport. This ensures that the browser gets redirected from `http` to an `https` link automatically.
