...
You need to specify a key string to be used when you encode by using AES encryption. This string is hashed to produce an encryption key that is used to encrypt and decrypt the password.
The key string can be provided from `bootstrapProperties` in the `pom.xml` file.
