...
It is not recommended to store passwords in plain text. The passwords in the [hotspot file=0]`userRegistry.xml` file are encoded by using the Liberty `securityUtility` command with XOR encoding. To learn more about the `securityUtility` commands, see the https://openliberty.io/docs/latest/reference/command/securityUtility-commands.html[securityUtility commands^] documentation.
