...
It is not recommended to store passwords in plain text. For simplicity reasons, the application in this guide uses XOR encoding to encrypt the passwords, as you can see in the `userRegistry.xml`. 
An example of how to encode the password using Liberty's securityUtility will be shown at the end of the guide.
...
----
=== Encrypting passwords using securityUtility (Optional)

The `securityUtility` command and `encode` parameter encodes provided plain text.
In order to encode 'bobpwd', navigate to the `target/liberty/wlp/bin` directory and run the Liberty `securityUtility` command:
```
