...
`http://localhost:9080`
As you can see, the browser gets redirected from `http` to an `https` link automatically, this is accomplished through the transport guaranteed that will be mentioned later on in the guide.
...
----
include::finish/src/main/java/io/openliberty/guides/ui/HomeServlet.java[tags=security;!copyright]
----
...
This constraint specifies url-patterns that are restricted to users that belong in certain roles.
The url under `url-pattern` is mapped to whichever role is specified under `role-name`. 
This role name must match the roles defined in the `server.xml` when configuring the user registry.
...
First, the method uses an HttpClient to execute the URL that is passed in by HTTP. After hitting this URL, the response should return the login.html.
Next, we use Java EE's `j_security_check`, as mentioned above, to login. If the test was meant to fail, the method checks whether or not the response page is the `error.html` page. 
