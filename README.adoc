...
The `user` and `admin` pages display user information. Both pages will show the name of the currently authenticated user and the admin page will also show the role of the user.
...
You can access the application pages by signing in to the `HomeServlet` with one of the credentials from the Identity Store . 
...
Open the `src/main/java/io/openliberty/guides/ui/UserBean.java` file:

[source, java, indent=0]
----
include::finish/src/main/java/io/openliberty/guides/ui/UserBean.java[tags=security;!copyright]
----
