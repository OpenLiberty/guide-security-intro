...
The [hotspot=45-48]`testAuthorizationForAdmin()` and [hotspot=51-54]`testAuthorizationForUser()` methods, respectively, verify that a user with [hotspot=47]`admin` role and a user with [hotspot=53]`user` role are properly authenticated and can access authorized resource.
